...
DataTable.ext.renderer.pageButton.uikit = function ( settings, host, idx, buttons, page, pages ) {
	...
	var attach = function( container, buttons ) {
		...
		for ( i=0, ien=buttons.length ; i<ien ; i++ ) {
			...
			if ( $.isArray( button ) ) {
				...
				switch ( button ) {
					case 'ellipsis':
						btnDisplay = '<i classes="uk-icon-ellipsis-h"></i>';
						btnClass = 'uk-disabled disabled';
						break;

					case 'first':
						btnDisplay = '<i classes="uk-icon-angle-double-left"></i> ' + lang.sFirst;
						btnClass = (page > 0 ?
							'' : ' uk-disabled disabled');
						break;

					case 'previous':
						btnDisplay = '<i classes="uk-icon-angle-left"></i> ' + lang.sPrevious;
						btnClass = (page > 0 ?
							'' : 'uk-disabled disabled');
						break;

					case 'next':
						btnDisplay = lang.sNext + ' <i classes="uk-icon-angle-right"></i>';
						btnClass = (page < pages-1 ?
							'' : 'uk-disabled disabled');
						break;

					case 'last':
						btnDisplay = lang.sLast + ' <i classes="uk-icon-angle-double-right"></i>';
						btnClass = (page < pages-1 ?
							'' : ' uk-disabled disabled');
						break;
	...
	attach(
		$(host).empty().html('<ul classes="uk-pagination uk-pagination-right uk-flex-right"/>').children('ul'),
		buttons
	);
